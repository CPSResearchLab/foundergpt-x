...
import { completeAutoSignIn, confirmEmailSignUp, getSession } from "@/services/auth";
...
function ConfirmSignUpForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await confirmEmailSignUp(values.data.email, values.data.code);
      if (response.nextStep.signUpStep === "COMPLETE_AUTO_SIGN_IN") {
        const signInResponse = await completeAutoSignIn();
        if (!signInResponse.isSignedIn) {
          setError(`Cognito requires an additional sign-in step: ${signInResponse.nextStep.signInStep}.`);
          return;
        }
      }
      const session = await getSession();
      if (!session.tokens) {
        setError("Account confirmation succeeded, but Cognito did not create a session. Return to login and check the browser console for the complete auth trace.");
        return;
      }
      router.replace("/dashboard");
      router.refresh();
    } catch (caughtError: unknown) {
      setError(caughtError instanceof Error ? `[${caughtError.name}] ${caughtError.message}` : "[UnknownAuthError] Unable to confirm your account. Check the browser console for details.");
    } finally {
    ...
    <main className="flex min-h-screen items-center justify-center bg-[#050914] px-5 py-12 text-white">
      <section className="glass w-full max-w-md rounded-2xl p-6 sm:p-8">
        ...
        <form onSubmit={submit} noValidate className="mt-8 space-y-5">
          ...
          <Button type="submit" disabled={isSubmitting || !isCognitoConfigured} className="h-12 w-full rounded-xl bg-white font-semibold text-slate-950 hover:bg-cyan-50">{isSubmitting ? "Confirming..." : "Confirm account"}</Button>
        </form>
